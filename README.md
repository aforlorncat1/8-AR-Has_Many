...
*Figure 1*.  Schema design for this exercise, showing connections between primary keys and foreign keys.
 ...
 Working with the schema shown in Figure 1, in the previous exercise we wrote a few belongs to associations for our models:
...
In this exercise we'll take a look at the *has many* association.  A has many association is the inverse of the belongs to association; it's the other side of a one-to-many relationship.  Taking the belongs to associations that we wrote for our models, we can write inverse has many associations:
...
We'll explore some of these methods in this exercise.  For a more comprehensive list and description of the methods provided, read the [API Dock description](http://apidock.com/rails/ActiveRecord/Associations/ClassMethods/has_many).
...
### Conclusion
As has been said in other exercises, Active Record associations are going to play a big role in our applications as we move through Recode.  The has many association is one of the major association types. We need to be familiar with it: both how to declare the association and how to employ the methods provided to us when we make the declaration.
